 ...
 * calculate the contravariant Maxwell
 */
void BsqmEsq(const double pr[NPR],
	     const struct of_geom *geom,double *BsqmEsq)
{
      int j ;

      double F1t, F2t, F3t, Mt1, Mt2, Mt3 ;    /* prims */
      double Econ[NDIM], Ecov[NDIM] ;          /* E four-vector */
      double Bcon[NDIM], Bcov[NDIM] ;          /* B four-vector */
      double beta[NDIM] ;                      /* shift  3-vector  */
      ...
      lower_v(Bcon, Bcov, geom) ;
      
      *BsqmEsq = 0.0;
      SLOOPA *BsqmEsq+=Bcon[j]*Bcov[j]-Econ[j]*Ecov[j];
